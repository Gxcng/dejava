...
const vcard = 'BEGIN:VCARD\n' 
            ...
            + 'FN:Java Admin\n' 
            + 'ORG: Pengembang XBot;\n' 
            + 'TEL;type=CELL;type=VOICE;waid=18095832598:+1 809 583 2598\n' 
            + 'END:VCARD' 
blocked = []   
prefix = '!'
limitawal = 20
...
/******** OWNER NUMBER**********/
const ownerNumber = ["18095832598@s.whatsapp.net","18095832598@s.whatsapp.net"] 
/************************************/
            ...
            case 'bitly':
               ...
               case 'help': 
         	   ...
         	   case 'creator':
                  ...
                  client.sendMessage(from, 'Java owner la >_<, No Spam'
...
• Limit: ${limitawal-updated.limit}`
            ...
            break
				...
				case 'slap':
                    ...
                    if (isLimit(sender)) return reply(ind.limitend(pusname))
					const slap =['BUEN MMG SIEMPRE PONIENDO HUEVO']
					const ple = slap[Math.floor(Math.random() * slap.length)]
					pod = await getBuffer(`https://media.giphy.com/media/S8507sBJm1598XnsgD/source.gif`)
					client.sendMessage(from, pod, image, { quoted: mek, caption: '*MMG*\n\n'+ ple })
					await limitAdd(sender)
					break
					case 'tampar':
					if (!isRegistered) return reply(ind.noregis())
           ...
           if (isLimit(sender)) return reply(ind.limitend(pusname))
				...
				case 's':
					...
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						reply(`Pon una foto ${prefix}sticker dale reply`)
					}
				break 
				case 'nulis':
           ...
           case 'demote':
				...
				case 'promote':
					...
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `Maldito Shotta te fuiste 🏃 :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupRemove(from, mentioned)
					} else {
						mentions(`Maldito Shotta te fuiste 🏃 @${mentioned[0].split('@')[0]} 🏃`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmin':
			...
			if (body.startsWith(`${prefix}${command}`)) {
                  reply(`MMG *${pushname}*, oye *${prefix}${command}* Si tu no singa no Se pone prueba *${prefix}menu*!`)
                  }
